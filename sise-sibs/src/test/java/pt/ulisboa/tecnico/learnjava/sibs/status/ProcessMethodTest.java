...
public class ProcessMethodTest {
	...
	int value = 100;
	Sibs sibs;
	private State withdrawn = Withdrawn.getInstance();
	private State deposited = Deposited.getInstance();
	private State cancelled = Cancelled.getInstance();
	...
	@Test
	public void processRegisteredFail() throws AccountException, ServicesException {
	...
	@Test
	public void processRegisteredRetried3Times() throws AccountException, ServicesException {
		this.transferOperation.setNumberOfRetries();
		this.transferOperation.setNumberOfRetries();
		this.transferOperation.setNumberOfRetries();
		when(this.mockService.canWithdraw(this.sourceIban, this.value)).thenReturn(false);
		this.transferOperation.getState().process(this.transferOperation, this.sibs);
	...
	@Test
	public void processWithdrawnFail() throws AccountException, ServicesException, OperationException {
		TransferOperation transferOperation1 = new TransferOperation(this.sourceIban, this.targetIban1, this.value);
		transferOperation1.setState(this.withdrawn);
	...
	@Test
	public void processWithdrawnRetried3Times() throws AccountException, ServicesException, OperationException {
		TransferOperation transferOperation1 = new TransferOperation(this.sourceIban, this.targetIban1, this.value);
		transferOperation1.setState(this.withdrawn);
		transferOperation1.setNumberOfRetries();
		transferOperation1.setNumberOfRetries();
		transferOperation1.setNumberOfRetries();
		when(this.mockService.canDeposit(this.targetIban, this.value)).thenReturn(false);
	...
	@Test
	public void processDepositedFail() throws OperationException, AccountException, ServicesException {
		TransferOperation transferOperation1 = new TransferOperation(this.sourceIban, this.targetIban1, this.value);
		transferOperation1.setState(this.deposited);
	...
	@Test
	public void processDepositedRetried3Times() throws AccountException, ServicesException, OperationException {
		TransferOperation transferOperation1 = new TransferOperation(this.sourceIban, this.targetIban1, this.value);
		transferOperation1.setState(this.deposited);
		transferOperation1.setNumberOfRetries();
		transferOperation1.setNumberOfRetries();
		transferOperation1.setNumberOfRetries();
		when(this.mockService.canWithdraw(this.sourceIban, transferOperation1.commission())).thenReturn(false);
		when(this.mockService.canWithdraw(this.targetIban1, transferOperation1.getValue())).thenReturn(true);
		when(this.mockService.canDeposit(this.sourceIban, this.value)).thenReturn(true);
		transferOperation1.getState().process(transferOperation1, this.sibs);
